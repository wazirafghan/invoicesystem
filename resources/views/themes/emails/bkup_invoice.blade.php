...
<header>
    ...
    <address contenteditable>
        <h3><b class="text-danger">{{$invoice->from_name}}</b></h3>
        <p> {{$invoice->from_phone}}</p>
        <p> {{$invoice->from_email}}</p>
        <p> {{$invoice->from_address}}</p>
    </address>
    @if($invoice->pic)
        <span><img alt="" width="160px" src="{{asset("uploads/invoice/$invoice->pic")}}"><input type="file" accept="image/*"></span>
    @endif
</header>
<article>
    ...
    <table class="meta">
        <tr>
            ...
            <td><span contenteditable>{{$invoice->invoice_num}}</span></td>
        </tr>
        <tr>
    ...
    <table class="inventory">
        <thead>
        <tr>
            <th><span contenteditable>Detail</span></th>
        ...
        @foreach($invoice->items as $item)
            <tr>
                <td><span contenteditable>{{$item->detail}}</span></td>
                <td><span contenteditable>{{$item->description}}</span></td>
                <td><span contenteditable>{{$item->qty}}</span></td>
                <td><span contenteditable>{{$item->rate}}</span></td>
                <td><span data-prefix>$</span><span contenteditable><?php echo $sub_total = $item->qty * $item->rate;?></span></td>
    ...
    <table class="balance">
        <tr>
            ...
            <td><span data-prefix>$</span><span>{{$net_total}}</span></td>
        </tr>
        <tr>
            <th><span contenteditable>Discount</span></th>
            <td><span data-prefix></span><span>{{$invoice->discount or 0}}</span></td>
        </tr>
        <tr>
